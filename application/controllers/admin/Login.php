...
class Login extends Admin_Controller {
	...
	public function doLogin(){
		...
		$password = md5($this->input->post('password',true).'GW');
		...
		if(isset($_SESSION['captcha_img']) && $captcha ==$_SESSION['captcha_img']){
			...
			if($account_res>0){
				...
				if($user_res){
					...
					if($this->input->post('is_cache')==true){
						$_SESSION['login'] = $user_res;
						$this->session->mark_as_temp('login',3600 * 24 * 8);
					}else{
						$_SESSION['login'] = $user_res;
					}
					$this->ajaxReturn(['retcode'=>1,'msg'=>'','data'=>['admin_type'=>$user_res['admin_type']]]);
				}else{
